...
func newTestScriptExecutor(t *testing.T) *workflow.ScriptExecutor {
	t.Helper()
	repoRoot := t.TempDir()
	env := map[string]string{
		"JERRY_SECRET_TEST": "secret_value",
	}
	return workflow.NewScriptExecutor(repoRoot, env)
}

func TestScriptCanExecute(t *testing.T) {
	exec := newTestScriptExecutor(t)
...
func TestScriptExecute_Success(t *testing.T) {
	exec := newTestScriptExecutor(t)
...
func TestScriptExecute_Failure(t *testing.T) {
	exec := newTestScriptExecutor(t)
...
func TestScriptExecute_Timeout(t *testing.T) {
	exec := newTestScriptExecutor(t)
...
func TestScriptExecute_StepName(t *testing.T) {
	exec := newTestScriptExecutor(t)
