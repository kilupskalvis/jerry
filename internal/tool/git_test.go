...
func TestGitLog_Success(t *testing.T) {
	...
	gitTool := tool.NewGitLogTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{}))
	if err != nil {
...
func TestGitLog_WithPath(t *testing.T) {
	repoRoot := setupGitRepo(t)
	gitTool := tool.NewGitLogTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{"path": "hello.go"}))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !strings.Contains(result, "initial commit") {
...
func TestGitLog_CountClamping(t *testing.T) {
	repoRoot := setupGitRepo(t)
	gitTool := tool.NewGitLogTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{"count": 100}))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result == "" {
...
func TestGitLog_NotARepo(t *testing.T) {
	gitTool := tool.NewGitLogTool(t.TempDir())
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{}))
	if err != nil {
...
func TestGitDiff_NoChanges(t *testing.T) {
	repoRoot := setupGitRepo(t)
	gitTool := tool.NewGitDiffTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{}))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGitDiff_WithRef(t *testing.T) {
	...
	gitTool := tool.NewGitDiffTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{"ref": "HEAD"}))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGitBlame_Success(t *testing.T) {
	repoRoot := setupGitRepo(t)
	gitTool := tool.NewGitBlameTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{"path": "hello.go"}))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGitBlame_PathTraversal(t *testing.T) {
	repoRoot := setupGitRepo(t)
	gitTool := tool.NewGitBlameTool(repoRoot)
	result, err := gitTool.Execute(context.Background(), mustJSON(t, map[string]any{"path": "../../etc/passwd"}))
	if err != nil {
		t.Fatalf("unexpected Go error: %v", err)
	}
