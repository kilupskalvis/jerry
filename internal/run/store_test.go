...
func newTestRunState(runID string) run.RunState {
...
func newStepResult(name string, status run.StepStatus) run.StepResult {
...
func TestFileStore_SaveCheckpoint(t *testing.T) {
	...
	runState.StepResults = append(runState.StepResults, newStepResult("step-one", run.StepSuccess))
...
func TestFileStore_SaveFinal(t *testing.T) {
	...
	if loaded.Status != run.StatusCompleted {
		t.Errorf("Status = %q, want %q", loaded.Status, run.StatusCompleted)
	}
...
func TestAtomicWriteJSON(t *testing.T) {
	...
	if err := run.AtomicWriteJSON(path, data); err != nil {
...
func TestAtomicWriteJSON_OverwriteExisting(t *testing.T) {
	...
	// Write first version
	if err := run.AtomicWriteJSON(path, map[string]int{"v": 1}); err != nil {
		t.Fatalf("first write error: %v", err)
	}

	// Overwrite with second version
	if err := run.AtomicWriteJSON(path, map[string]int{"v": 2}); err != nil {
