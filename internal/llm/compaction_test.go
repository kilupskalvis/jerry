...
func (m *mockSummarizer) Send(_ context.Context, _ string, _ []Message, _ []ToolDef) (*Response, error) {
	m.calls++
	if m.err != nil {
		return nil, m.err
	}
...
func buildTestMessages(count int) []Message {
	...
	for i := range count {
		...
		case 0:
			msgs = append(msgs, Message{
				...
				ToolCalls: []ToolCall{
					{ID: fmt.Sprintf("call_%d", i), Name: "read_file", Arguments: map[string]any{"path": "test.go"}},
				},
			})
		case 1:
			msgs = append(msgs, Message{
...
func TestCompact_BasicCompaction(t *testing.T) {
	...
	compactor := NewCompactor(mock)

	messages := buildTestMessages(20)
	result, err := compactor.Compact(context.Background(), "system prompt", messages, DefaultKeepRecent)
	if err != nil {
	...
	if result.CompactedMessages[0].Role != RoleUser {
		t.Errorf("first compacted message role = %q, want %q", result.CompactedMessages[0].Role, RoleUser)
	}
	if result.Summary == "" {
...
func TestCompact_NothingToCompact(t *testing.T) {
	...
	_, err := compactor.Compact(context.Background(), "system", messages, DefaultKeepRecent)
	if err == nil {
...
func TestCompact_PreservesRecentMessages(t *testing.T) {
	...
	result, err := compactor.Compact(context.Background(), "system", messages, DefaultKeepRecent)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
