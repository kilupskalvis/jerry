...
func (m *mockSummarizer) Complete(_ context.Context, _ llm.CompleteParams) (*llm.CompleteResponse, error) {
	m.calls++
	if m.err != nil {
		return nil, m.err
	}
...
func buildTestMessages(count int) []llm.Message {
...
func TestCompact_BasicCompaction(t *testing.T) {
	...
	compactor := llm.NewCompactor(mock)

	messages := buildTestMessages(20)
	result, err := compactor.Compact(context.Background(), "system prompt", messages, llm.DefaultKeepRecent)
	if err != nil {
	...
	if result.CompactedMessages[0].Role != llm.RoleUser {
		t.Errorf("first compacted message role = %q, want %q", result.CompactedMessages[0].Role, llm.RoleUser)
	}
	if result.Summary == "" {
...
func TestCompact_NothingToCompact(t *testing.T) {
	...
	_, err := compactor.Compact(context.Background(), "system", messages, llm.DefaultKeepRecent)
	if err == nil {
...
func TestCompact_PreservesRecentMessages(t *testing.T) {
	...
	result, err := compactor.Compact(context.Background(), "system", messages, llm.DefaultKeepRecent)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
